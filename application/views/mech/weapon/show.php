...
<?php if(count($weapon_profile)):  ?>  <!-- za svakog korisnika (redak) - sve vrijednosti (članove niza) -->
		...
		<tr><td>Vlasnik: <?php echo $weapon_profile->vlasnik ?></td> </tr>
																<!-- potreban id vlasnika-->
		<tr><td>Zadnji aktivni servis: <?php if($weapon_profile->pripadni_servis!=NULL){ echo anchor('mech/service/show/' . $weapon_profile->pripadni_servis, $weapon_profile->naziv_servisa);} ?></td> </tr>
		...
		<tr>
			<td>Servisi: <?php 
						...
						foreach($services as $service_profile): ?>
							...
							|<?php echo anchor('mech/service/show/' . $service_profile->id, $service_profile->naziv);?>|&nbsp
